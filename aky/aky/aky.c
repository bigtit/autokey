...
LRESULT CALLBACK DlgProc(HWND hdlg, UINT msg, WPARAM wp, LPARAM lp)
{
  ...
  switch(msg){
    case WM_INITDIALOG:
      ...
      if(SetHook(mode, atoi(itv), vk)){
        if(mode==1){
          ...
          lstrcat(tip, skey);
          SetSkey(vsk);
          SetTip(tip, "开关: 关闭", hdlg, &ballon);
        }
        else if(mode==2) SetTip(tip, "按住", hdlg, &ballon);
        else SetTip(tip, "关闭", hdlg, &ballon);
      }
      else SetTip(tip, "关闭", hdlg, &ballon);
    ...
    case WM_TRAY:
      ...
      switch(mode){
        case 0: SetTip(tip, "关闭", hdlg, &ballon); break;
        case 1:
          lstrcat(tip, "\n");
          lstrcat(tip, skey);
          SetTip(tip, "开关", hdlg, &ballon);
          break;
        case 2: SetTip(tip, "按住", hdlg, &ballon); break;
        default: break;
      }
	    }
      break;
      ...
      if(wid==IDC_BUTTON1){
        ...
        if(SetHook(mode, atoi(itv), vk)){
          if(mode==1){
            ...
            if(!mode) SetTip(tip, "关闭", hdlg, &ballon);
            else SetTip(tip, "按住", hdlg, &ballon);
    ...
    case WM_ONOFF:
      lstrcat(tip, "\n");
      lstrcat(tip, skey);
      if(!wp) SetTip(tip, "开关:关闭", hdlg, &ballon);
      else SetTip(tip, "开关:开启", hdlg, &ballon);
      break;
    case WM_EDITSHOW:
